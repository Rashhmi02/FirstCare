...
export default function AddMedicine() {
  ...
  const tokens =JSON.parse(localStorage.getItem('userToken'));
        ...
        Axios.post(`${host}/api/medi/medicineInsert`, medicineDetails,{headers:{'auth-token':tokens}})
            .then((res) => {
                if (res.data) {
                    setOpen(true);
                    setMedicineDetails({
    ...
    return (
        <div style={{ minheight: '100vh', padding:'6px'}}>
            <Paper>
                <Box sx={{ backgroundColor: '#DEECFF', padding: '2px 15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <p style={{ color: 'rgb(82, 95, 127)', fontWeight: '400', fontSize: '15px' }}>Insert Medicine Details</p>
